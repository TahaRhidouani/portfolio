...
export function SleepText({visible, direction}: { visible: boolean, direction: "right" | "left" }) {
    ...
    useGSAP(() => {
        ...
        const anchors = [{"x": d * 100, "y": -100}, {"x": d * -50, "y": -100}, {"x": d * 100, "y": -200}]
        ...
        for (let i = 0; i < 3; i++) {
            timeline.current.fromTo(".text-" + (i + 1), {
    ...
    return (
        <div ref={texts} className={styles.sleepText} style={{opacity: visible ? 1 : 0, transition: "opacity 0.2s"}}>
            <h1 className="text-1" style={{position: "absolute"}}>Z</h1>
            <h1 className="text-2" style={{position: "absolute"}}>Z</h1>
            <h1 className="text-3" style={{position: "absolute"}}>Z</h1>
        </div>
    );
}
